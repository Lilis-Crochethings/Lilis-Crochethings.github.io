  ...
  return chart.steps.map((step, stepIndex): RenderRow => {
    ...
    if (step.parts?.length) {
      const repeats = step.parts.map((part, partIndex) => ({
        ...
        segments: resolveSegments(part.line, part.color, step.color ?? chart.color, textColorToYarnId),
        total: part.total,
      ...
      return {
        ...
        anchorId: step.id,
        labelRange: label,
        segments: resolveSegments(step.line, step.color, chart.color, textColorToYarnId),
        total: step.total,
